...
public class SystemFactoryIosLogin {
    private SystemFactoryIosLogin() {}

    public static UserSystem onlyIosLogin(Tuple3<UserRes, NewAccTokenData, NewRefTokenData> tuple3) {
        UserRes user = tuple3.getT1();
        return new UserSystem(
                user.getSys().getCredStars(),
                user.getSys().getAccC(),
                user.getSys().getAccC(),
                Security.iosLogin(tuple3)
        );
    }
}
